...
if (isset($_GET['id'])) {
    ...
    try {
        $check_id = $connectDB->prepare($check_id_SQL);
        $check_id->bindParam(":cid", $cid);
        $check_id->execute(); #คิวรี
        if ($result = $check_id->fetch(PDO::FETCH_ASSOC)) {
            echo "error";
        } else {
            echo "success";
        }
...
if (isset($_GET['email'])) {
    ...
    try {
        $check_email = $connectDB->prepare($check_email_SQL);
...
if (isset($_POST["data"])) {
    ...
    $Inser_Member = $connectDB->prepare($sql);
